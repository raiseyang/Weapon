 ...
 * LLog.w(TAG, " w");
 * LLog.e(TAG, " e = " + null);
 * LLog.d(TAG, " %s,%d", "raise", 1);
 * LLog.i(TAG, " %s,%d", "raise", 1);
 * LLog.w(TAG, " %s,%d", "raise", 1);
 * LLog.e(TAG, new NullPointerException("fu*k null pointer exception."));
 * LLog.json(TAG, "{\"name\":\"BeJson\",\"url\":\"http://www.bejson.com\",\"page\":88,\"isNonProfit\":true}");
 * LLog.array(TAG, new String[]{"value1", "value2"});
 * LLog.list(TAG, Arrays.asList("list1", "list2", "list3"));
 * LLog.xml(TAG,"<student><age>12</age><name>jack</name><skill><language>chinese</language><run>22</run></skill></student>");
 * LLog.file(TAG, new File(Environment.getExternalStorageDirectory().getAbsolutePath()+"/test.log"));
 */
public class LLog {
    ...
    private static final ListenerHandlerThread mHandlerThread; // 将日志发到子线程写入

    // 用于缓存日志文件大小
    private static final String SP_KEY_LOG_SIZE = "weapon:sp_key_log_size";
    ...
    /**
     * 设置打印日志级别
     * 设置为{@link LLog#NONE}不输出日志
     *
     * @param level 默认：{@link LLog#DEBUG}
     */
    ...
    /**
     * 日志文件的绝对路径:/sdcard/xxx/weapon_log.txt
     */
    public static String getLogPath() {
        return LLog.s_log_path;
    }
    ...
    public static void e(String tag, String msg, Throwable tr) {
        println(6, tag, msg + '\n' + getStackLLogString(tr));
    }

    public static void e(String tag, Throwable tr) {
        println(6, tag, getStackLLogString(tr));
    }
    ...
    /**
     * Handy function to get a loggable stack LLog from a Throwable
     *
     * @param tr An exception to log
     */
    private static String getStackLLogString(Throwable tr) {
        if (tr == null) {
            return "";
        }
    ...
    private static boolean check_log_file() {
        ...
        if (!log_file.exists()) {
            if (!log_file.getParentFile().exists()) {
                ...
                if (!mkdirs) {
                    Log.e(s_global_tag, getStackLLogString(new IOException("Can't create the directory of LLog. Please check the LLog path:"
                            + log_file.getParent())));
                    return false;
                }
            }
